...
def random_crop_image(image_name, text, image_size, class_num, phase, idx, no_aug, args):
    ...
    h,w = image.size
    # print 'h',h,'w',w
    h = int(float(h) * image_size[1] / w)#image_size[64,64]
    ...
    # 填充成固定大小
    image = np.transpose(np.array(image), [2,0,1]).astype(np.float32)
    # print 'image_name:',image_name,'image.shape:',image.shape
    if image.shape[2] < image_size[0]:
        ...
        new_image_temp=new_image[:,:,start,np.newaxis]
        # print 'new_image_temp.shape',new_image_temp.shape
        new_image[:,:,0:start] = new_image_temp
        # print '=============!!!!'
        new_image_temp=new_image[:,:,start+image.shape[2]-1,np.newaxis]
        new_image[:,:,start+image.shape[2]:] = new_image_temp
        if phase == 'test':
